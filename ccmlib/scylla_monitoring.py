    ...
    if isinstance(dashboard, dict):
        for key in list(dashboard.keys()):
            ...
            if key == 'datasource':
                ...
                elif isinstance(val, dict) and val.get('name') == 'prometheus':
                    # Dashboard schema v2 (kind: DataQuery) refs the
                    # datasource by "name", and Grafana treats that value
                    # as the uid when normalising for the API/legacy
                    # panels -- an extra "uid" key here is ignored.
                    val['name'] = _DATASOURCE_UID
    ...
    def is_running(self):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=2) as resp:
                # Check the body, not just the status: some other service
                # (e.g. Cockpit also defaults to 9090) may answer 200 on
                # this port, which would otherwise be a false positive.
                body = resp.read(64)
                if resp.status == 200 and b'Prometheus' in body:
                    return True
        except Exception:
            pass
