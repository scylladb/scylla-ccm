...
class LogPatternToVersion(object):
    ...
    def __call__(self, version):
        ...
        return self.versions_to_patterns[
            max(keys_less_than_version, key=lambda v: ComparableCassandraVersion(v) if not isinstance(v, ComparableCassandraVersion) else v)]
...
def is_modern_windows_install(version):
    ...
    Handles floats, strings, and Versions by first converting all three types to a string, then to a Version.
    """
    version = ComparableCassandraVersion(str(version))
    if is_win() and version >= ComparableCassandraVersion('2.1'):
        return True
    else:
        return False
        ...
        if is_win():
            if ':' not in install_dir:
                raise ArgumentError(f'{install_dir} does not appear to be a cassandra or dse installation directory.  '
                                    f'Please use absolute pathing (e.g. C:/cassandra.')
    ...
    if install_dir is not None:
        ...
        if (dse_version is not None):
            if cassandra:
                ...
                return ComparableCassandraVersion(dse_version)
        ...
        with open(build) as f:
            for line in f:
                ...
                if match:
                    return ComparableCassandraVersion(match.group(1))
...
def get_dse_version(install_dir):
    for root, dirs, files in os.walk(install_dir):
        for file in files:
            match = re.search(r'^dse(?:-core)?-([0-9.]+)(?:-.*)?\.jar', file)
            if match:
...
def get_supported_jdk_versions(cassandra_version, install_dir, for_build, env):
    ...
    if cassandra_version and not isinstance(cassandra_version, ComparableCassandraVersion):
        cassandra_version = ComparableCassandraVersion(cassandra_version)
    ...
    if build_versions is None:
        if cassandra_version and cassandra_version >= ComparableCassandraVersion('5.0'):
            build_versions = [11, 17]
            run_versions = [11, 17]
            info("Cassandra 5.0+ detected, using Java 11 and 17")
        elif cassandra_version and cassandra_version >= ComparableCassandraVersion('4.0'):
