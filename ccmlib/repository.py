...
def setup(version, verbose=False):
    ...
    if cdir is None:
        try:
            ...
            logger.warning("Downloading %d failed, due to %s. Trying to build from git instead.", version, err)
...
def clone_development(git_repo, version, verbose=False):
    logger.debug("Git repo is %s and the version is %s", git_repo, version)
    ...
    with open(logfile, 'w') as lf:
        try:
            ...
            if not os.path.exists(local_git_cache):
                if verbose:
                    logger.debug("Cloning Cassandra...")
                out = subprocess.call(
                ...
                if verbose:
                    logger.debug("Fetching Cassandra updates...")
                out = subprocess.call(
            ...
            if not os.path.exists(target_dir):
                ...
                if verbose:
                    logger.debug("Cloning Cassandra (from local cache)")
                ...
                if str(status).find('[behind') > -1:
                    if verbose:
                        logger.debug("Branch is behind, recompiling")
            ...
            try:
                rmdirs(target_dir)
                logger.debug("Deleted %s due to error", target_dir)
            except:
...
def download_dse_version(version, username, password, verbose=False):
    ...
    try:
        ...
        if verbose:
            logger.debug("Extracting %s as version %s ...", target, version)
...
def download_opscenter_version(version, target_version, verbose=False):
    ...
    try:
        ...
        if verbose:
            logger.debug("Extracting %s as version %s ...", target, target_version)
        tar = tarfile.open(target)
        dir = tar.next().name.split("/")[0]
        tar.extractall(path=__get_dir())
        tar.close()
...
def download_version(version, url=None, verbose=False, binary=False):
    ...
    try:
        ...
        # wipe out the directory if anything goes wrong. Otherwise we will assume it has been compiled the next time
        # it runs.
        try:
            rmdirs(target_dir)
            logger.debug("Deleted %s due to error", target_dir)
        except:
            raise CCMError("Building C* version %s failed. Attempted to delete %s but failed. This will need to be"
                           " manually deleted" % (version, target_dir))
        raise e
...
def compile_version(version, target_dir, verbose=False):
    ...
    logfile = lastlogfilename()
    if verbose:
        logger.debug("Compiling Cassandra %s ...", version)
    with open(logfile, 'w') as lf:
...
def __download(url, target, username=None, password=None, show_progress=False):
    ...
    if show_progress:
        logger.debug("Downloading %s to %s (%.3fMB)", url, target, float(file_size) / (1024 * 1024))
