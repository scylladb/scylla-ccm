...
class ScyllaDockerNode(ScyllaNode):
    ...
    def create_docker(self, args):
        ...
        if not self.pid:
            try:
                ...
                # Check if host filesystem supports O_DIRECT
                use_tmpfs_data = False
                if client.runtime_name == 'podman':
                    result = run(['stat', '-f', '-c', '%T', self.get_path()],
                                stdout=PIPE, stderr=PIPE, universal_newlines=True)
                    if result.stdout.strip() in ('zfs', 'fuse'):
                        use_tmpfs_data = True
                        LOGGER.info(f"Host filesystem ({result.stdout.strip()}) doesn't support O_DIRECT, "
                                    "using tmpfs for data directories")

                # Data directories that need O_DIRECT support
                data_dirs = [d for d in self.share_directories if d != 'logs']
                tmpfs_mounts = []

                if use_tmpfs_data:
                    # Use tmpfs for data dirs (supports O_DIRECT),
                    # bind-mount only logs for host access
                    logs_path = os.path.join(self.get_path(), 'logs')
                    volumes[logs_path] = os.path.join(self.base_data_path, 'logs')
                    for directory in data_dirs:
                        container_path = os.path.join(self.base_data_path, directory)
                        tmpfs_mounts.append(container_path)
                else:
                ...
                # Build a custom entrypoint that fixes permissions and
                # configures supervisor before launching it. This avoids
                # the slow stop/chown/restart cycle.
                mount_paths_str = ' '.join(
                    ['/etc/scylla'] + [os.path.join(self.base_data_path, d) for d in self.share_directories]
                )
                entrypoint_script = (
    ...
    def chmod(self, file_path, permissions):
        ...
        try:
            ...
            runtime = self.get_container_client().runtime_name
            run(['bash', '-c',
                 f'{runtime} run --rm -v {file_path}:{path_inside_docker} busybox chmod -R {permissions} {path_inside_docker}'],
                stdout=PIPE, stderr=PIPE)
...
class DockerLogger:
    ...
    def _logger_cmd(self) -> str:
        runtime = self._node.get_container_client().runtime_name
        return f'{runtime} logs -f {self._node.pid} >> {self._target_log_file} 2>&1 &'
