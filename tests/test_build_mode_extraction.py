...
def test_scylla_mode_extraction_from_install_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        install_dir = Path(tmpdir) / "scylla-install"
...
def test_scylla_cluster_init_extracts_mode_from_install_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        install_dir = Path(tmpdir) / "scylla-install-debug"
        core_package_dir = install_dir / CORE_PACKAGE_DIR_NAME
        core_package_dir.mkdir(parents=True, exist_ok=True)
...
def test_scylla_cluster_timeout_settings():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for mode, expected_notice_timeout, expected_binary_timeout in test_cases:
            install_dir = Path(tmpdir) / f"scylla-install-{mode}"
            core_package_dir = install_dir / CORE_PACKAGE_DIR_NAME
            core_package_dir.mkdir(parents=True, exist_ok=True)
