...
class TestRunContainerExtendedArgs:
    ...
    def test_run_container_never_relabels_tmp(self, mock_run, mock_which):
        """/tmp is opted out of the :z suffix on podman (some SELinux
        policies refuse to relabel it, and it's a shared system path)."""
        mock_which.return_value = '/usr/bin/podman'
        client = PodmanClient()
        mock_run.reset_mock()
        mock_run.return_value.stdout = 'container123'
