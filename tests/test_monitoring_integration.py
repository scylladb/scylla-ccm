...
class TestMonitoringLifecycle:
        ...
        for entry in search:
            ...
            for ref in refs:
                if isinstance(ref, str):
                    ...
                    # Skip Grafana built-ins ('datasource' picker meta-type,
                    # '-- Grafana --' annotations source) and the optional
                    # scylla-datasource CQL trace plugin: none of these are
                    # provisioned by our stack, and none are Prometheus.
                    if ds_type in ('datasource', 'grafana', 'scylladb-scylla-datasource'):
                        continue
    ...
    def test_08b_grafana_dashboards_have_data(self, cluster, ports):
        ...
        ds_uid = prom_ds[0]['uid']

        # Fetch a dashboard and extract real PromQL expressions from panels
        search = http_get_json(f"{grafana}/api/search?type=dash-db")
        assert search, "No dashboards found in Grafana"

        # Pull queries from every dashboard, not just the first one found:
        # some dashboards (e.g. "Advanced") only have scylla-manager panels,
        # which have no data in a manager-less test cluster, so restricting
        # to a single dashboard risks an all-empty sample by bad luck.
        panel_queries = []
        ...
        # Query each expression through Grafana's datasource proxy. Uses the
        # uid-based resource proxy: the legacy numeric-id proxy API
        # (/api/datasources/proxy/:id/...) was removed in current Grafana.
        proxy_base = f"{grafana}/api/datasources/uid/{ds_uid}/resources"

        def query_all():
            found = []
            for _, panel_title, expr in panel_queries:
                ...
                results = data.get('data', {}).get('result', [])
                if results:
                    found.append((panel_title, expr, len(results)))
            return found

        # Retry: under load, Prometheus may not have scraped/aggregated
        # enough samples yet for rate()-based panel queries.
        queries_with_data = []
        deadline = time.time() + 30
        while time.time() < deadline:
            queries_with_data = query_all()
            if queries_with_data:
                break
            time.sleep(2)
